...
#pragma omp parallel for
  ...
  // first A -> B
  for (int i = 0; i < num_A_donors; i++) {
    // if donor inside pore
    if (  (donors.pos(i)[2] >= hbpara.min_z) && (donors.pos(i)[2] <= hbpara.max_z )  &&  
	  ( ( donors.pos(i)[0]*donors.pos(i)[0] + donors.pos(i)[1]*donors.pos(i)[1] ) <=  hbpara.radius2 ) ) {
      gmath::Vec bound_i = pbc->nearestImage(donors.pos(i), bound.pos(i),
					     sys -> box());
      for (int j = num_A_acceptors; j < acceptors.size(); j++)
	if (  (acceptors.pos(j)[2] >= hbpara.min_z) && (acceptors.pos(j)[2] <= hbpara.max_z )  &&  
	   ( ( acceptors.pos(j)[0]*acceptors.pos(j)[0] + acceptors.pos(j)[1]*acceptors.pos(j)[1] ) <=  hbpara.radius2 ) )
	  calc2c(i, j, bound_i);
    }
  }
#ifdef OMP
...
// added by JAG
void HB2c_calc::printstatistics_pore(vector<int> loads_in_time) {
  int max = *std::max_element(loads_in_time.begin(),loads_in_time.end());
  // vector of gmath::Stat<double> to store hbonds in function of loads
  std::vector<gmath::Stat<double> > hbond_loads(max+1);
  std::cout << "# Statistics of the run:" << endl;
  std::cout << endl << "# Two-centered hydrogen bonds:\n";
  std::cout << "#"
          << setw(8) << "HB"
  ...
  std::vector<int>::const_iterator iter;
  std::vector<int> tmp;
  timeseriesHBtot << setw(15) << "#Time";
  timeseriesHBtot << setw(10) << "Hbonds";
  timeseriesHBtot << setw(10) << "Load"<<endl;
  for (unsigned int ia = 0; ia < tstimeHB.size(); ia++) {
    timeseriesHBtot.precision(9);
    timeseriesHBtot << setw(15) << tstimeHB[ia];
    timeseriesHBtot << setw(10) << tsnumHB[ia];
    timeseriesHBtot << setw(10) << loads_in_time[ia] << endl;
    hbond_loads[loads_in_time[ia]].addval(tsnumHB[ia]);
    
  }
  // Dirty hack, instead saving time series of each h-bond
  // we save the average hbonds in function of the pore loads
  timeseriesHB<<"#Average Hbonds in function of pore loading"<<endl;
  timeseriesHB<<setw(16)<<"#Load";
  timeseriesHB<<setw(16)<<"<Hbonds/Load>";
  timeseriesHB<<setw(16)<<"error";
  timeseriesHB<<setw(16)<<"rmsd"<<endl;
  for (int i = 1; i<hbond_loads.size(); i++) {
    timeseriesHB.precision(9);
    timeseriesHB<<setw(16)<<i;
    timeseriesHB<<setw(16)<<(hbond_loads[i].ave())/i;
    timeseriesHB<<setw(16)<<(hbond_loads[i].ee())/i;
    timeseriesHB<<setw(16)<<(hbond_loads[i].rmsd())/i<<endl;
  }
    
  
}//end HB2c_calc::printstatistics_pore()
